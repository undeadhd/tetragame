...
class GameScene: SKScene {
  ...
  var level: Level!
    ...
    let CookieWidth: CGFloat = 36.0
    let CookieHeight: CGFloat = 36.0
  ...
  override init(size: CGSize) {
    ...
    TileWidth = (size.width - 30.0) / 6.0
    TileHeight = TileWidth
    
    anchorPoint = CGPoint(x: 0.5, y: 0.5)
    let background = SKSpriteNode(color: UIColor.blue , size: size)
  ...
  func addSprites(for cookies: Set<Cookie>) {
    for cookie in cookies {
      ...
      sprite.size = CGSize(width: CookieWidth, height: CookieHeight)
...
//      let texture = SKTexture(imageNamed: cookie.cookieType.highlightedSpriteName)
      selectionSprite.size = CGSize(width: CookieWidth, height: CookieHeight)
